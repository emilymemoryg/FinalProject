...
def LoadOtherEntry(ChatLog, EntryText,username):
    if EntryText != '':
        ChatLog.config(state=NORMAL)
        if ChatLog.index('end') != None:
            try:
            ...
            ChatLog.tag_add(username, LineNumber, LineNumber+0.6)
            ChatLog.tag_config(username, foreground="#04B404", font=("Arial", 12, "bold"))
            ChatLog.config(state=DISABLED)
            ChatLog.yview(END)
